...
func (m Manager) InstallDependenciesOnServer(request InstallDependenciesOnServerRequest, ctx context.Context, _ context.CancelFunc) error {
	...
	detectedOS, err := ssh_toolkit.DetectOS(5, server.IP, server.SSHPort, server.User, m.Config.SystemConfig.SshPrivateKey)
	if err != nil {
	...
	for _, dependency := range core.RequiredServerDependencies {
		...
		if dependency != "init" {
			...
			err = ssh_toolkit.ExecCommandOverSSH(core.DependencyCheckCommands[dependency], stdoutBuffer, nil, 5, server.IP, server.SSHPort, server.User, m.Config.SystemConfig.SshPrivateKey)
			if err == nil {
		...
		if isExists {
			...
			err = ssh_toolkit.ExecCommandOverSSH(command, stdoutBuffer, stderrBuffer, 5, server.IP, server.SSHPort, server.User, m.Config.SystemConfig.SshPrivateKey)
